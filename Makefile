...
EXCLUDED_TARGETS := $(sort $(notdir $(patsubst %/,%,$(dir $(wildcard $(ROOT)/src/main/target/*/.exclude)))))

# Targets for CI
CI_TARGETS       := $(filter-out $(EXCLUDED_TARGETS), $(BASE_TARGETS))
...
TARGET_FLAGS  	+= -D$(TARGET) -D$(TARGET_MCU_FAMILY)
...
$(TARGETS_CLEAN):
	$(V0) $(MAKE) -j $(JFLAG) TARGET=$(subst _clean,,$@) clean

## <CONFIG>_clean    : clean up one specific target (alias for above)
$(CONFIGS_CLEAN):
	$(V0) $(MAKE) -j $(JFLAG) CONFIG=$(subst _clean,,$@) clean
...
dfu_erase_flash dfu_erase_and_flash: $(TARGET_DFU)
ifneq (no-port-found,$(SERIAL_DEVICE))
	$(V0) echo -n 'R' > $(SERIAL_DEVICE) ; sleep 5
endif
...
binary:
	$(V0) $(MAKE) -j $(JFLAG) $(TARGET_BIN)

hex:
	$(V0) $(MAKE) -j $(JFLAG) $(TARGET_HEX)
